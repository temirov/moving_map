...
UNZIP_DIR=$(realpath "../src/tl_2023_us_county")  # Get the absolute path
ZIP_FILE="../src/tl_2023_us_county.zip"
ENV_FILE="../../.env"
DOCKER_IMAGE="postgis:utils"
host_ip=$(hostname -I | awk '{print $1}')
TABLE_NAME="us_counties"

CREATE_INDEX_SQL="CREATE INDEX idx_us_counties_geom ON $TABLE_NAME USING GIST (geom);"

CREATE_MATERIALIZED_VIEW="CREATE MATERIALIZED VIEW IF NOT EXISTS us_counties_centroids AS
SELECT gid, ST_Centroid(geom)::geometry(Point, 4326) AS geom, namelsad AS county_name
FROM us_counties;
"
...
sh -c '/usr/bin/shp2pgsql -I -s 4326 /data/tl_2023_us_county.shp $TABLE_NAME > /data/shp2pgsql_output.sql'
...
# Step 3c: Creating MATERIALZIED VIEW
echo "Creating MATERIALZIED VIEW from the table..."
execute_sql "$CREATE_MATERIALIZED_VIEW"
echo "MATERIALZIED VIEW created."
